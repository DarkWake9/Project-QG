...
for grb in GRBs:
    ...
    with dyn.pool.Pool(mul.cpu_count(), loglklhood_null_HP, prior_transform) as Pool:
    ...
    # Linear LIV
    with dyn.pool.Pool(mul.cpu_count(), loglklhood_LIV_lin, prior_transform_LIV_lin) as Pool:
    ...
    # Quadratic LIV
    with dyn.pool.Pool(mul.cpu_count(), loglklhood_LIV_quad, prior_transform_LIV_quad) as Pool:
    ...
    def smooth_plot(results, figname, labels=["logE_qg", "logEb", "alpha1", "alpha2", "mu", "zeta"]):
        ...
        plt.savefig(os.getcwd() + '/outputs/contours/' + figname + '.png')
        plt.show()
    ...
    plt.savefig(os.getcwd() + '/outputs/fit/' + grbname_wtht_ext + '_fit_logE.png', facecolor='white')
    plt.show()
    ...
    with open(os.getcwd() + f'/outputs/BF/{grbname_wtht_ext}_BF.txt', 'w') as f:
        ...
        f.write([bayes_factor_lin, bayes_factor_quad])
