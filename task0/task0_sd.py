...
#        Eb = Eb_max * Eb
        alpha1v = alpha1_min + (alpha1_max - alpha1_min) * alpha1
        alpha2v = alpha2_min + (alpha2_max - alpha2_min) * alpha2
#        mu = mu_max * mu
#        zeta = zeta_max * zeta
        
        return [Eb_max*Eb, alpha1v, alpha2v, mu_max*mu, zeta_max*zeta]
    ...
    with Pool() as pool:
        sampler0 = NestedSampler(loglklhood_null_HP, prior_transform, ndim=ndim_NULL, bound='multi', sample='unif', queue_size=10,pool=pool,bootstrap=0)
    ...
    # Linear LIV
    with Pool() as pool:
        sampler1 = NestedSampler(loglklhood_LIV_lin, prior_transform_LIV_lin, ndim=ndim_LIV, bound='multi', sample='unif', pool=pool, queue_size=10)
    ...
    # Quadratic LIV
    with Pool() as pool:
        sampler2 = NestedSampler(loglklhood_LIV_quad, prior_transform_LIV_quad, ndim=ndim_LIV, bound='multi', sample='unif', pool=pool, nlive=nlive,queue_size=10)
